    ...
    <div
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div class="sm:col-span-2">
          ...
          <div class="relative">
            <input v-model="filters.search" type="text" placeholder="Rechercher par lieu..."
    ...
    <div class="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 mx-4 sm:mx-0">
      ...
      <div v-else-if="viewMode === 'grid'" class="p-3 sm:p-4 lg:p-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          <div v-for="inspection in filteredInspections" :key="inspection.id" @click="goToDetail(inspection.uuid)"
      ...
      <div v-else class="divide-y divide-gray-200">
        <div v-for="inspection in filteredInspections" :key="inspection.id" @click="goToDetail(inspection.id)"
          ...
          <div class="flex items-center justify-between gap-2 sm:gap-3">
            ...
            <div class="flex items-center space-x-1 sm:space-x-2 flex-shrink-0">
              <button @click.stop="downloadReport(inspection.id)"
      ...
      <div v-if="filteredInspections.length > 0" class="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200">
        <div class="flex items-center justify-between">
          <p class="text-xs sm:text-sm text-gray-600">
            ...
            <span class="hidden sm:inline"> sur <span class="font-medium">{{ filteredInspections.length }}</span>
              résultats</span>
          </p>
        </div>
      </div>
...
const filteredInspections = computed(() => {
  ...
  if (filters.value.search) {
    ...
    result = result.filter(
      (i) =>
        i.lieu.toLowerCase().includes(searchLower) ||
  ...
  if (filters.value.statut) {
    result = result.filter((i) => i.statut === filters.value.statut)
  }

  // Filtre par date de début
  if (filters.value.dateFrom) {
    result = result.filter((i) => new Date(i.date_inspection) >= new Date(filters.value.dateFrom))
  }

  // Filtre par date de fin
  if (filters.value.dateTo) {
    result = result.filter((i) => new Date(i.date_inspection) <= new Date(filters.value.dateTo))
  }
...
const goToDetail = (uuid) => {
  router.push({ name: 'InspectionDetail', params: { uuid } })
}
