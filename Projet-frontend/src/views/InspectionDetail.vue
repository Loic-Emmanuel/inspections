          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p class="text-base font-semibold text-gray-900 mt-1">{{ formatDate(inspection.date) }}</p>
              </div>
              <div>
                <label class="text-sm font-medium text-gray-500">Éleveur</label>
                <p class="text-base font-semibold text-gray-900 mt-1">{{ inspection.farmer_name }}</p>
              </div>
              <div>
              ...
              <div v-if="inspection.farming_type">
                ...
                <p class="text-base font-semibold text-gray-900 mt-1">{{ inspection.farming_type }}</p>
              </div>
              <div v-if="inspection.animal_count">
                <label class="text-sm font-medium text-gray-500">Nombre d'animaux</label>
                <p class="text-base font-semibold text-gray-900 mt-1">{{ inspection.animal_count }}</p>
              </div>
            </div>
          </div>

          <!-- Observations -->
          <div v-if="inspection.observations" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h2 class="text-xl font-bold text-gray-900 mb-4">Observations</h2>
            <p class="text-gray-700 leading-relaxed whitespace-pre-line">{{ inspection.observations }}</p>
          </div>

          <!-- Recommandations -->
          <div v-if="inspection.recommendations" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h2 class="text-xl font-bold text-gray-900 mb-4">Recommandations</h2>
            <p class="text-gray-700 leading-relaxed whitespace-pre-line">{{ inspection.recommendations }}</p>
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="text-center">
              ...
              >
                <svg v-if="inspection.compliance === 'Conforme'" class="w-12 h-12 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                </svg>
                <svg v-else-if="inspection.compliance === 'Non conforme'" class="w-12 h-12 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p class="text-lg font-bold text-gray-900">{{ inspection.compliance || 'Non définie' }}</p>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                ...
                <span class="text-lg font-bold text-gray-900">{{ inspection.images_count || 0 }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm text-gray-600">Catégories</span>
                <span class="text-lg font-bold text-gray-900">{{ inspection.categories?.length || 0 }}</span>
              </div>
              <div class="flex items-center justify-between">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div v-if="inspection.categories && inspection.categories.length > 0" class="space-y-8">
          <div v-for="category in inspection.categories" :key="category.id">
            ...
            <div v-if="category.images.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              <div
