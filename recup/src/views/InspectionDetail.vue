  ...
  <MainLayout>
    ...
    <div v-if="loading" class="flex items-center justify-center py-20">
      <div class="text-center">
        <div class="inline-block animate-spin rounded-full h-12 w-12 border-4 border-gray-300 border-t-indigo-600 mb-4">
        </div>
    ...
    <div v-else-if="inspection">
      ...
      <div class="flex items-center justify-between mb-8">
        <div>
          <button @click="$router.back()" class="flex items-center text-gray-600 hover:text-gray-900 mb-3 transition">
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
            Retour
          </button>
          <h1 class="text-3xl font-bold text-gray-900">{{ inspection.lieu }}</h1>
          <div class="flex items-center space-x-4 mt-2">
            <span class="px-3 py-1 text-sm font-medium rounded-full" :class="getStatusClass(inspection.statut)">
              {{ inspection.statut }}
            </span>
            <span class="text-gray-600">{{ formatDate(inspection.date_inspection) }}</span>
          </div>
        </div>
        ...
        <div class="flex items-center space-x-3">
          ...
          <button class="p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition" title="Partager">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                <label class="text-sm font-medium text-gray-500">Date de l'inspection</label>
              ...
              <div>
                ...
                <p class="text-base font-semibold text-gray-900 mt-1">{{ inspection.lieu }}</p>
              </div>
          ...
          <!-- Observations -->
          <div v-if="inspection.description" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="text-center">
              <div class="w-24 h-24 mx-auto rounded-full flex items-center justify-center mb-3"
                :class="getComplianceColor(inspection.statut)">
                <svg v-if="inspection.statut === 'Conforme'" class="w-12 h-12 text-green-600" fill="none"
                  stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                </svg>
                <svg v-else-if="inspection.compliance === 'Non conforme'" class="w-12 h-12 text-red-600" fill="none"
                  stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                ...
                <span class="text-lg font-bold text-gray-900">{{ inspection.nbreImage || 0 }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm text-gray-600">Catégories</span>
                <span class="text-lg font-bold text-gray-900">{{ nombre_categories || 0 }}</span>
              </div>
              <div class="flex items-center justify-between">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div v-if="inspection.images && inspection.images.length" class="space-y-8">

          <!-- Catégories uniques -->
          <div v-for="category in [...new Set(inspection.images.map(i => i.categories))]" :key="category">
            <!-- Header catégorie -->
            <div class="flex items-center justify-between mb-4">
              ...
              <span class="text-sm text-gray-600">
                {{
                  inspection.images.filter(img => img.categories === category).length
                }} image{{inspection.images.filter(img => img.categories === category).length > 1 ? 's' : ''}}
              </span>
            </div>

            <!-- Images -->
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              <div v-for="(image, index) in inspection.images.filter(img => img.categories === category)"
                :key="image.id" @click="openLightbox(
                  inspection.images.filter(img => img.categories === category),
                  index
                )"
                class="relative group cursor-pointer overflow-hidden rounded-lg border border-gray-200 hover:border-indigo-400 transition">
                <img :src="`${API_BASE_BASE_URL}/storage/${image.image_path}`" :alt="`Image ${index + 1}`"
    ...
    <Teleport to="body">
      <div v-if="lightboxOpen" class="fixed inset-0 z-50 bg-black bg-opacity-90 flex items-center justify-center"
        ...
        <button v-if="currentLightboxImages.length > 1" @click.stop="previousImage"
        ...
        <div class="max-w-6xl max-h-screen p-8" @click.stop>
          <img :src="`${API_BASE_BASE_URL}/storage/${currentLightboxImages[currentLightboxIndex]?.image_path}`"
            alt="Image en grand" class="max-w-full max-h-[85vh] object-contain rounded-lg" />
        ...
        <!-- Navigation suivante -->
        <button v-if="currentLightboxImages.length > 1" @click.stop="nextImage"
